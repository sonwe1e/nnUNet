...
class DC_and_Weight_BCE_loss(nn.Module):
    def __init__(
        self,
        bce_kwargs,
        soft_dice_kwargs,
        ...
        :param aggregate:
        """
        super(DC_and_Weight_BCE_loss, self).__init__()
        if use_ignore_label:
    ...
    def forward(
        ...
        dc_loss = self.dc(
            net_output, target_regions, loss_mask=mask, loss_weight=loss_weight
        )
        if mask is not None:
            ...
            ce_loss = (
                self.ce(net_output, target_regions)
                * loss_weight.reshape(-1, 1, 1, 1, 1)
            ).mean()
        result = self.weight_ce * ce_loss + self.weight_dice * dc_loss
        return result
